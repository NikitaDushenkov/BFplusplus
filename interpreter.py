    ...
    def build_bracket_map(self):
        ...
        for i, char in enumerate(self.code):
            if char == "[":
                ...
                if not stack:
                    raise SyntaxError("Unmatched closing bracket at position {}".format(i))
                start = stack.pop()
                bracket_map[start] = i
                bracket_map[i] = start
        if stack:
            raise SyntaxError("Unmatched opening bracket at position {}".format(stack.pop()))
        return bracket_map
    ...
    def run(self):
        ...
        while self.code_pointer < len(self.code):
            ...
            if command == ">":
                ...
                end_index = code.find('}', self.code_pointer)
                if end_index == -1:
                    raise SyntaxError("Unmatched '{'")
                module_name = code[self.code_pointer + 1:end_index]
                if module_name.isnumeric():
                    module_name = int(module_name)
                    if module_name in self.modules:
                        self.data[self.data_pointer] = ord(self.run_module(module_name))
                    else:
                        raise ValueError(f"Module with ID {module_name} not found.")
                else:
